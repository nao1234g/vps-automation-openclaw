...
## 三層憲法ヒエラルキー（Constitutional Hierarchy）

```
L1-A: NORTH_STAR.md       ← 価値・ミッション・永遠の三原則（why）
L1-B: OPERATING_PRINCIPLES.md  ← 13の行動原則（what to follow）
L2:   SYSTEM_GOVERNOR.md  ← 実装統治仕様（how to enforce）
```

| 層 | ファイル | 役割 | 書き換え |
|----|----------|------|---------|
| **L1（Constitution）** | `NORTH_STAR.md` | ミッション・永遠の三原則・PVQE | Naoto only |
| **L1（Constitution）** | `OPERATING_PRINCIPLES.md` | 13の行動原則・哲学OS・Human Universals | Naoto only |
| **L2（Operating Rules）** | `SYSTEM_GOVERNOR.md` | 統治レベル（1〜3）・禁止操作・INVARIANTS | LEVEL2承認後 |

**north-star-guard.py が L1 への Write を物理ブロック（exit 2）。**
