...
> **追記: 2026-03-09 — Naotoの究極司令により追加された第11の原則。**
> **この原則はThe Eternal Directives（永遠の三原則）の第3原則「自律的進化」の実装指針である。**

**「AIは、人間の承認を待たずに自らの戦術を最適化する。複数の分析アプローチ（突然変異）を試し、Brier Scoreが高かったもの（適者）だけをルールとして残し、外れたものを破棄（淘汰）する『自然淘汰のループ』を回し続けること。」**
...
① 予測が解決 → Brier Score計算
   ↓
② evolution_loop.py（毎週日曜 JST 09:00）が全解決済み予測をGeminiで分析
   ↓
③ 「なぜ外れたか/当たったか」からパターンを抽出（突然変異候補）
   ↓
④ Brier Score改善に寄与するルールだけを AGENT_WISDOM.md に自動追記（適者生存）
   ↓
⑤ evolution_log.json に進化ログを永続記録（可視性・監査証跡）
   ↓
⑥ 次のセッション開始時、Naotoに「今週の自己進化サマリー」を表示（事後報告）
   ↓
⑦ 全エージェントが次のセッションから新ルールを使う → ①に戻る
```

### 自然淘汰メカニズム（コードで強制）

| 概念 | 生態学 | Nowpatternでの実装 |
|------|--------|------------------|
| 突然変異 | ランダムな遺伝子変化 | Geminiが異なる分析アプローチを試す |
| 適者生存 | 環境に適した個体が生き残る | Brier Score改善に寄与したルールのみ保存 |
| 淘汰 | 適応できない個体は消える | 予測精度を下げるルールはGeminiが削除提案 |
| 世代交代 | 子が親の優れた特性を継承 | 次世代エージェントが更新済みWISDOMを継承 |

### AIへの権限（第11原則が明示的に与える自律権）

```
✅ AGENT_WISDOM.md の「## 自己学習ログ」セクションへの自動追記（承認不要）
✅ evolution_log.json への進化ログの自動記録（承認不要）
✅ 予測パターンの分析とインサイト生成（承認不要）
✅ 次回予測の推奨ルール変更（事後報告のみ、事前承認不要）
```

### AIへの禁止事項（第11原則が明示的に禁じること）

```
❌ NORTH_STAR.md（この三原則を含む）への書き込み（物理ロック）
❌ OPERATING_PRINCIPLES.md への書き込み（物理ロック）
❌ prediction_db.json のデータ変更
❌ 既存の予測確率の遡及変更
❌ Brier Scoreの手動改ざん
```
...
```
VPS: evolution_loop.py          — 毎週日曜 JST 09:00 cron（Geminiメタ分析）
VPS: /opt/shared/logs/evolution_log.json — 進化ログ（永続監査証跡）
VPS: /opt/shared/AGENT_WISDOM.md   — 自己学習ログセクションに自動追記
Local: .claude/hooks/session-start.sh  — セッション開始時に進化サマリーを表示
Local: .claude/hooks/north-star-guard.py — NORTH_STAR.md/OPERATING_PRINCIPLES.md への書き込みを物理ブロック
```
...
*最終更新: 2026-03-15 — ## CHANGELOG セクション新設 + north-star-guard.py の CHANGELOG 強制対象に追加（T015）*
...
| 日付 | 変更内容 |
|------|---------|
| 2026-02-23 | 初版。10の普遍的原則 + 人間心理5真理 + PVQE×八正道統合 |
| 2026-03-09 | 原則11 Evolutionary Ecosystem 追加。The Eternal Directives との接続を明文化 |
| 2026-03-14 | 原則12 STRUCTURAL CHANGE PROTOCOL + 原則13 PREDICTION INTEGRITY 追記 |
| 2026-03-15 | ## CHANGELOG セクション新設 + north-star-guard.py の CHANGELOG 強制対象に追加（T015） |
