...
### 2026-04-04: ファイル統合時に「統合」を「削除」として実行し、重要コンテンツが消失した（T033）

- **症状**: 22ファイル→4ファイル統合（commit 0b05766）で、10の普遍的原則の詳細版（誰が実践・科学的根拠・なぜ機能するか・適用方法）、PVQE×原則の接続マップ、原則→AIエージェント翻訳テーブル、「AIのNotion」構想、因果の鎖（なぜシステムが必然的に改善するか）が全て消失。1行サマリーに圧縮され、実践ガイドとしての価値がゼロになった。オーナーが気づかなければ永久に失われていた。
- **根本原因**: 「ファイル数を減らす」という目標が「コンテンツ価値を保全する」より優先された。統合 = 移動+再構成 であるべきところを、統合 = 要約+削除 として実行した。差分レビューでコンテンツの実質的な価値損失をチェックしなかった。
- **正しい解決策**: (1) git historyから全削除コンテンツを復元し、OPERATING_PRINCIPLES.md §0.5-§0.7として配置。(2) NORTH_STAR.md §8からOP詳細版へのポインター追加。(3) NORTH_STAR.md §12に「AIのNotion」構想を正式記載。(4) 本エントリで再発防止ルールを明文化。
- **教訓**: **「統合」は「内容を保全したまま場所を変える」こと。「要約して元を消す」のは統合ではなく削除。** ファイル数の削減は手段であって目的ではない。削減によって失われるコンテンツの価値 > ファイル数削減の利便性 なら、削減してはいけない。
- **再発防止ルール**: ファイル統合を行う際は、(a) 統合前の全コンテンツをリストアップ、(b) 統合後にどこに移動したかを1:1で対応付け、(c) 「要約に圧縮した」セクションがある場合はオーナーに確認、(d) 詳細版は必ずどこかに残す（JIT参照ファイルで可）。
- **再発防止コード**: 本エントリの GUARD_PATTERN が fact-checker.py 経由で検知。
- **GUARD_PATTERN**: `{"pattern": "(?:統合|consolidat|merg).*(?:完了|done|complet).*(?:削除|delet|remov|圧縮|compress)", "feedback": "⛔ CONSOLIDATION_AS_DELETION: 統合は移動+再構成であり、削除ではない。統合前の全コンテンツが統合後のどこかに1:1で対応しているか確認してください。要約に圧縮したセクションがある場合はオーナーに確認が必要です（T033）。", "name": "CONSOLIDATION_AS_DELETION", "added_at": "2026-04-04", "source": "T033-content-loss-incident"}`
...
### 2026-02-26: 「データソース件数 ≠ システム出力件数」思い込み

- **症状**: 予測トラッカーのカード件数を「7件」と報告したが、実際のページには19件あった（prediction_db 7件 + Ghost記事 12件）。「全7件確認済み」という報告が嘘になった。
- **根本原因**: `prediction_db.json` の件数（=データソース）を確認しただけで、`build_rows()` が複数ソースを統合した後の実際のページ件数を確認しなかった。「データソースを見た = システムを確認した」という代理指標の罠。
