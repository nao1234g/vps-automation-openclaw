    ...
    const disposable = vscode.commands.registerCommand('codespacesConnector.connect', async () => {
        ...
        try {
            ...
            if (!codespacesExt) {
                ...
                const choice = await vscode.window.showWarningMessage('GitHub Codespaces extension is not installed. Would you like to install it?', 'Install', 'Cancel');
                if (choice === 'Install') {
            ...
            if (!session) {
                ...
                const authChoice = await vscode.window.showInformationMessage('GitHub authentication is required to connect to Codespaces.', 'Sign in', 'Cancel');
            ...
            if (codespaces.length === 0) {
                ...
                const createChoice = await vscode.window.showInformationMessage('No Codespaces found. Would you like to create one?', 'Create', 'Cancel');
            ...
            if (codespaceName) {
                ...
                vscode.window.showInformationMessage(`Successfully connected to Codespace: ${codespaceName}`);
            }
            else {
                outputChannel.appendLine('⚠️  Connection status unclear (may still be connecting)');
                statusBarItem.text = `$(info) Connecting...`;
                vscode.window.showInformationMessage('Connection initiated. Please wait for the window to reload.');
            }
